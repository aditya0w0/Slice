...
@section("content")
    ...
    >
        ...
        >
            <!-- Header / Logo -->
            <div
                class="h-16 flex items-center justify-between px-4 border-b border-white/5"
            >
                <div
                    class="flex items-center gap-3 overflow-hidden whitespace-nowrap"
                >
                    <div
                        class="flex-shrink-0 w-10 h-10 rounded-xl bg-gradient-to-br from-blue-600 to-purple-600 flex items-center justify-center shadow-lg shadow-blue-500/20"
                    >
                        <svg
                ...
                <button
                    ...
                    class="absolute -right-3 top-20 bg-blue-600 text-white rounded-full p-1 shadow-lg hover:bg-blue-500 transition-colors z-50 border border-[#0B0C10]"
                >
                    <svg
            ...
            >
                ...
                <div
                    x-show="sidebarOpen"
                    class="px-3 mb-2 text-xs font-bold text-slate-500 uppercase tracking-wider transition-opacity duration-200"
                >
                    Operations
                </div>
                ...
                <a
                    ...
                    class="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:bg-white/5 hover:text-white transition-all"
                >
                    <div
                ...
                <!-- Inbox (Active) -->
                <div
                    class="group flex items-center gap-3 px-3 py-2.5 rounded-xl bg-blue-600/10 text-white border border-blue-600/20 relative"
                >
                    ...
                    <div
                        class="flex-shrink-0 w-6 h-6 flex items-center justify-center ml-1"
                    >
                        <svg
                    ...
                    <span
                        x-show="sidebarOpen"
                        class="ml-auto bg-blue-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-md"
                    >
                        4
                    </span>
                </div>
                ...
                >
                    ...
                    >
                        <svg
                            class="w-5 h-5 group-hover:text-purple-400 transition-colors"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                        >
                ...
                >
                    ...
                    >
                        <svg
                            class="w-5 h-5 group-hover:text-yellow-400 transition-colors"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                        >
            ...
            <div class="p-4 border-t border-white/5">
                <div class="relative">
                    <button
                        ...
                        class="flex items-center gap-3 w-full p-2 rounded-xl hover:bg-white/5 transition-colors group"
                    >
                        @php
                            $admin = Auth::user();
                        ...
                        <img
                            src="{{ $adminAvatar }}"
                            class="w-8 h-8 rounded-full ring-2 ring-white/10 group-hover:ring-white/30 transition-all"
                            alt=""
                        />
                    ...
                    >
                        <form method="POST" action="{{ route("logout") }}">
                            ...
                            <button
                                type="submit"
                                class="w-full flex items-center gap-2 px-4 py-3 text-sm text-red-400 hover:bg-white/5 hover:text-red-300 transition-colors"
                            >
                                <svg
                                    class="w-4 h-4"
                                    fill="none"
        ...
        <div
            class="w-80 bg-[#0F1015] border-r border-white/5 flex flex-col z-20"
        >
            <!-- Header -->
            <div
                class="h-16 px-6 border-b border-white/5 flex items-center justify-between bg-[#0F1015]"
            >
            ...
            <div class="px-4 py-3">
                <div class="relative group">
                    ...
                    >
                        <svg
                            class="w-4 h-4 text-slate-500 group-focus-within:text-blue-500 transition-colors"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                        >
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar">
                @forelse ($contacts as $contact)
                    <div
                        ...
                        class="group px-4 py-3 border-b border-white/5 hover:bg-white/[0.02] cursor-pointer transition-all {{ isset($contact["active"]) ? "border-l-2 border-l-blue-500 bg-blue-500/5" : "border-l-2 border-l-transparent" }}"
                    >
                        <div class="flex items-start gap-3">
                            <div class="relative flex-shrink-0">
                                <img
                                    src="{{ $contact["avatar"] }}"
                                    alt=""
                                    class="w-10 h-10 rounded-full object-cover ring-2 ring-white/5 group-hover:ring-white/10 transition-all"
                                />
                                @if ($contact["status"] === "online")
                                    <div
                                        class="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 rounded-full border-2 border-[#0F1015]"
                                    ></div>
                                @endif
                            </div>
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p
                                    class="text-xs text-slate-400 truncate group-hover:text-slate-300 transition-colors {{ $contact["is_typing"] ? "font-medium text-green-400" : "" }}"
                                >
                            ...
                            @if ($contact["unread"] > 0)
                                <div class="flex-shrink-0 self-center ml-2">
                                    <span
                                        class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-blue-600 text-[10px] font-bold text-white shadow-lg shadow-blue-600/30"
                                    >
                                        {{ $contact["unread"] }}
                                    </span>
                                </div>
                ...
                @empty
                    <div
                        class="flex flex-col items-center justify-center h-64 text-slate-500 px-6 text-center"
                    >
                        <div
                            class="w-12 h-12 bg-white/5 rounded-full flex items-center justify-center mb-3"
                        >
                            <svg
        ...
        <div class="flex-1 flex flex-col bg-[#0B0C10] relative z-10">
            @if (isset($activeChat["user"]["id"]))
                <!-- Header -->
                <div
                    class="h-16 px-6 border-b border-white/5 flex items-center justify-between bg-[#0B0C10]/80 backdrop-blur-xl"
                >
                    <div class="flex items-center gap-4">
                        <div class="relative">
                            ...
                            />
                            <div
                                class="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 rounded-full border-2 border-[#0B0C10]"
                            ></div>
                        </div>
                        <div>
                            <h3
                            ...
                            >
                                ...
                                <span
                                    class="px-2 py-0.5 rounded-md bg-white/5 border border-white/5 text-[10px] text-slate-400 font-normal uppercase tracking-wide"
                                >
                                    Client
                                </span>
                            </h3>
                            <p
                                class="text-xs text-green-400 flex items-center gap-1.5 mt-0.5"
                            >
                                <span
                    ...
                    <div class="flex items-center gap-3">
                        <a
                            href="{{ route("admin.users.show", $activeChat["user"]["id"]) }}"
                            class="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs font-medium text-slate-300 hover:bg-white/10 hover:text-white transition-colors"
                        >
                            View Profile
                        </a>
                        <button
                ...
                <div
                    ...
                    class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAiIGhlaWdodD0iMjAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGNpcmNsZSBjeD0iMSIgY3k9IjEiIHI9IjEiIGZpbGw9InJnYmEoMjU1LDI1NSwyNTUsMC4wMykiLz48L3N2Zz4=')]"
                >
                    <div class="flex justify-center py-4">
                        <span
                            class="px-3 py-1 rounded-full bg-white/5 border border-white/5 text-[10px] font-medium text-slate-500"
                        >
                    ...
                    @foreach ($activeChat["messages"] as $message)
                        <div
                            class="flex {{ $message["sender"] === "me" ? "justify-end" : "justify-start" }} group"
                        >
                            <!-- Avatar if 'them' -->
                            @if ($message["sender"] === "them")
                                <img
                                    src="{{ $activeChat["user"]["avatar"] }}"
                                    class="w-8 h-8 rounded-full mr-3 mt-1 opacity-70 self-end mb-1"
                                />
                            @endif
            ...
            @else
                ...
                >
                    <div
                        class="w-24 h-24 bg-white/5 rounded-full flex items-center justify-center mb-6 relative"
                    >
                        <div
                            class="absolute inset-0 bg-blue-500/10 rounded-full animate-pulse"
                        ></div>
                        <svg
                    ...
                    <p class="text-slate-500 max-w-sm mt-2">
                        Select a conversation from the inbox to manage support
                        tickets and user inquiries.
                    </p>
                </div>
            @endif
        </div>
