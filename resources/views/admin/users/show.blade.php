...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white shadow-sm border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-6">
                ...
                <a href="{{ route('admin.users.index') }}" class="text-slate-600 hover:text-slate-700 font-medium">
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200 grid grid-cols-2 gap-4 text-center">
                        ...
                        <div>
                            <p class="text-2xl font-bold text-gray-900">{{ $user->credit_score ?? 0 }}</p>
                    ...
                    <div class="space-y-3 text-sm">
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-gray-900 font-medium">{{ $user->city }}{{ $user->state ? ', ' . $user->state : '' }} {{ $user->zip_code }}</p>
                                @endif
                            </div>
                        </div>
                        @endif
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="space-y-2">
                        <form action="{{ route('admin.users.toggleAdmin', $user) }}" method="POST">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="w-full px-4 py-2 text-left text-sm font-medium rounded-lg transition-colors {{ $user->is_admin ? 'bg-yellow-50 text-yellow-700 hover:bg-yellow-100' : 'bg-blue-50 text-blue-700 hover:bg-blue-100' }}">
                        ...
                        <form action="{{ route('admin.users.destroy', $user) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this user?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="w-full px-4 py-2 text-left text-sm font-medium bg-red-50 text-red-700 hover:bg-red-100 rounded-lg transition-colors">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            @if($user->legal_name)
                            <div>
                                <label class="text-sm font-medium text-gray-600">Legal Name</label>
                                <p class="mt-1 text-gray-900">{{ $user->legal_name }}</p>
                            </div>
                            @endif
                            @if($user->date_of_birth)
                            <div>
                                <label class="text-sm font-medium text-gray-600">Date of Birth</label>
                                <p class="mt-1 text-gray-900">{{ $user->date_of_birth->format('M d, Y') }}</p>
                            </div>
                            @endif
                            <div>
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-gray-900 font-semibold">${{ number_format($user->balance, 2) }}</p>
                            </div>
                            @endif
                            @if($user->referral_code)
                            <div>
                                <label class="text-sm font-medium text-gray-600">Referral Code</label>
                                <p class="mt-1 text-gray-900 font-mono">{{ $user->referral_code }}</p>
                            </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    <div class="px-6 py-4 border-b border-gray-200">
                        <div class="flex justify-between items-center">
                            ...
                            @if($totalOrders > 3)
                            <button onclick="showAllOrders()" class="text-sm text-blue-600 hover:text-blue-700">
                        ...
                        <div class="px-6 py-4 hover:bg-gray-50">
                            <div class="flex justify-between items-start">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 mt-1">{{ $order->created_at->format('M d, Y g:i A') }}</p>
                                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    <div class="px-6 py-4 border-b border-gray-200">
                        <div class="flex justify-between items-center">
                            ...
                            @if($totalLoginLogs > 3)
                            <button onclick="showAllLogins()" class="text-sm text-blue-600 hover:text-blue-700">
                    ...
                    <div class="divide-y divide-gray-200" id="logins-list">
                        ...
                        <div class="px-6 py-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1">
                                    <p class="text-sm text-gray-900">{{ $log->logged_in_at->format('M d, Y g:i A') }}</p>
                                    <p class="text-xs text-gray-600 mt-1">
                                        {{ $log->ip_address ?? 'Unknown IP' }} • {{ $log->user_agent ?? 'Unknown Device' }}
                                    </p>
                                </div>
                            </div>
                        </div>
    ...
    <!-- All Orders Modal -->
    <div id="orders-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[80vh] overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                <h3 class="text-lg font-semibold text-gray-900">All Orders for {{ $user->name }}</h3>
                <button onclick="closeOrdersModal()" class="text-gray-500 hover:text-gray-700">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>
            <div class="overflow-y-auto max-h-[60vh]" id="all-orders-container">
    ...
    <div id="logins-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[80vh] overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                <h3 class="text-lg font-semibold text-gray-900">All Login Activity for {{ $user->name }}</h3>
                <button onclick="closeLoginsModal()" class="text-gray-500 hover:text-gray-700">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>
            <div class="overflow-y-auto max-h-[60vh]" id="all-logins-container">
                <div class="text-center py-8">
                    <i class="fas fa-spinner fa-spin text-3xl text-gray-400"></i>
                    <p class="text-gray-600 mt-2">Loading login activity...</p>
                </div>
            </div>
        </div>
    </div>
    ...
    fetch('{{ route("admin.users.orders", $user) }}')
        ...
        .then(data => {
            ...
            if (data.orders.length === 0) {
                container.innerHTML = '<div class="px-6 py-8 text-center text-gray-500"><i class="fas fa-shopping-cart text-3xl mb-2"></i><p>No orders yet</p></div>';
                return;
            }
            ...
            data.orders.forEach(order => {
                ...
                else if (order.status === 'active') statusClass = 'bg-blue-100 text-blue-800';
        ...
        .catch(error => {
            ...
            document.getElementById('all-orders-container').innerHTML = '<div class="px-6 py-8 text-center text-red-500"><p>Failed to load orders</p></div>';
        });
}
...
function showAllLogins() {
    ...
    fetch('{{ route("admin.users.logins", $user) }}')
        .then(response => response.json())
        .then(data => {
            const container = document.getElementById('all-logins-container');
            if (data.logins.length === 0) {
                container.innerHTML = '<div class="px-6 py-8 text-center text-gray-500"><i class="fas fa-history text-3xl mb-2"></i><p>No login history available</p></div>';
                return;
            }
        ...
        .catch(error => {
            console.error('Error:', error);
            document.getElementById('all-logins-container').innerHTML = '<div class="px-6 py-8 text-center text-red-500"><p>Failed to load login activity</p></div>';
        });
}
