...
<html>
    ...
    <body>
        ...
        <main class="mx-auto max-w-7xl px-6 pb-32 lg:pb-20">
            @php
                // choose first image safely and ensure file exists
                $firstImage = "/images/product-iphone.svg";
                if (is_array($variants) && count($variants)) {
                    $candidate = $variants[0]->image ?? "";
                    if (! empty($candidate) && file_exists(public_path(ltrim($candidate, "/")))) {
            ...
            <div class="mt-8 grid gap-8 lg:grid-cols-12 lg:items-start">
                <div class="lg:col-span-7">
                    <div class="rounded-lg bg-white p-6">
                        ...
                        />
                        <div class="mt-6 grid grid-cols-4 gap-4 lg:grid-cols-5">
                            @foreach ($variants as $v)
                                @php
                                    $imgCandidate = $v->image ?? "";
                                    if (! empty($imgCandidate) && file_exists(public_path(ltrim($imgCandidate, "/")))) {
                                        $imgPath = $imgCandidate;
                                    } else {
                                        // fallback to a generic product image for this family
                                        $imgPath = "/images/product-iphone.svg";
                                        if (str_starts_with(mb_strtolower($base), "ipad")) {
                                            $imgPath = "/images/product-ipad.svg";
                                        } elseif (mb_stripos($base, "mac") !== false) {
                                            $imgPath = "/images/product-mac.svg";
                                        } elseif (mb_stripos($base, "watch") !== false || mb_stripos($base, "accessory") !== false) {
                                            $imgPath = "/images/product-watch.svg";
                                        }
                                    }
                                @endphp

                                <button class="thumb shrink-0 rounded border bg-white p-2" data-image="{{ $imgPath }}">
                                    <img src="{{ $imgPath }}" alt="{{ $v->name }}" class="h-24 w-24 object-contain" />
                                </button>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="relative z-10 w-full max-w-md rounded-lg bg-white p-6">
                        ...
                        <form id="inline-login-form" class="mt-4 space-y-4">
                            ...
                            <div class="flex items-center justify-end gap-3">
                                <button type="button" id="inline-login-cancel" class="rounded-md border px-4 py-2">
                                    Cancel
                                </button>
                                <button
                                    type="submit"
                                    id="inline-login-submit"
                                    class="rounded-md bg-blue-600 px-4 py-2 text-white"
                                >
                                    Sign in
                                </button>
                            </div>
                            <div id="inline-login-error" class="mt-2 hidden text-sm text-red-600"></div>
                        </form>
                    </div>
                </div>
                ...
                <div class="mt-4" id="rent-modal-body">
                    @auth
                        <p class="text-sm text-gray-700">
                            You're about to rent
                            <strong id="modal-variant-name"></strong>
                            .
                        </p>
                        <div class="mt-2 space-y-1">
                            <div>
                                <strong>Duration:</strong>
                                <span id="modal-months"></span>
                                months
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <span id="modal-price-monthly"></span>
                            </div>
                            <div>
                                <strong>Total:</strong>
                        ...
                        <div class="mt-6 flex justify-end gap-3">
                            ...
                            <button id="rent-confirm" class="rounded-md bg-blue-600 px-4 py-2 text-white">
                                Confirm & Rent
                            </button>
                        </div>
                    @endauth
                    ...
                    @guest
                        <p class="text-sm text-gray-700">
                            You're adding this item to your cart as a guest. You'll be asked to sign in at checkout to
                            complete the order.
                        </p>
                        <div class="mt-2 space-y-1">
                            <div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function capacityMultiplier(capacity) {
                    ...
                    if (capacity.indexOf('512') !== -1) return 1.1;
                    if (capacity.indexOf('1 TB') !== -1 || capacity.indexOf('1TB') !== -1) return 1.2;
                    return 1.0;
                }
                ...
                // (removed navbar update helpers) — navbar is intentionally unchanged by this page

                monthsSelect.addEventListener('change', updatePrices);

                // wire thumbnails via delegation to reduce listeners
                const thumbsContainer = document.querySelector('.mt-6.grid') || null;
                if (thumbsContainer) {
                ...
                function closeInlineLogin() {
                    ...
                    if (inlineLoginError) {
                        ...
                        inlineLoginError.classList.add('hidden');
                    }
                }

                if (inlineLoginOpen)
                    inlineLoginOpen.addEventListener('click', function (e) {
                        e.preventDefault();
                        openInlineLogin();
                    });
                        ...
                        try {
                            ...
                            if (res.ok) {
                                ...
                                const redirectUrl =
                                    ...
                                    (window.location.search ? window.location.search + '&openModal=1' : '?openModal=1');
                                ...
                                if (inlineLoginError) {
                                    inlineLoginError.textContent = msg;
                                    inlineLoginError.classList.remove('hidden');
                                }
                            } else {
                                const text = await res.text();
                                if (inlineLoginError) {
                                    inlineLoginError.textContent = 'Login failed';
                                    inlineLoginError.classList.remove('hidden');
                                }
                            }
                        } catch (err) {
                            console.error(err);
                            if (inlineLoginError) {
                                inlineLoginError.textContent = 'Network error';
                                inlineLoginError.classList.remove('hidden');
                            }
                        } finally {
                ...
                if (rentConfirm) {
                    rentConfirm.addEventListener('click', function () {
                        ...
                        const payload = {
                            ...
                            months:
                                rentMonths && rentMonths.value ? Number(rentMonths.value) : Number(monthsSelect.value),
