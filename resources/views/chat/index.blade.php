...
@section('content')
<div class="h-screen bg-[#0B0C10] flex overflow-hidden font-sans selection:bg-blue-500/30 selection:text-blue-100">
    ...
    <div class="w-80 bg-[#0F1015] border-r border-white/5 flex flex-col">
        ...
        <div class="p-4 border-b border-white/5">
            <div class="flex items-center justify-between mb-4">
                ...
                <button class="p-2 text-slate-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors">
            ...
            <div class="relative">
                ...
                >
                <svg class="w-5 h-5 text-slate-500 absolute left-3 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex-1 overflow-y-auto">
            ...
            <a href="{{ route('chat.index', ['userId' => $conversation['user']['id']]) }}" 
               class="flex items-center gap-3 px-4 py-3 hover:bg-white/5 transition-colors border-l-2 {{ $conversation['active'] ? 'border-blue-500 bg-white/5' : 'border-transparent' }}">
                <div class="relative flex-shrink-0">
                    <img src="{{ $conversation['user']['avatar'] }}" class="w-12 h-12 rounded-full border-2 border-white/10" alt="">
                    @if($conversation['user']['online'])
                    <span class="absolute bottom-0 right-0 w-3.5 h-3.5 bg-green-500 rounded-full border-2 border-[#0F1015]"></span>
                    @endif
                </div>
                <div class="flex-1 min-w-0">
                    <div class="flex items-center justify-between mb-1">
                        <h3 class="text-sm font-semibold text-white truncate">{{ $conversation['user']['name'] }}</h3>
                        <span class="text-xs text-slate-500">{{ $conversation['last_message_time'] }}</span>
                    </div>
                    <p class="text-xs text-slate-400 truncate">{{ $conversation['last_message'] }}</p>
                </div>
                @if($conversation['unread_count'] > 0)
                <span class="flex-shrink-0 px-2 py-0.5 bg-blue-600 text-white text-xs font-semibold rounded-full">{{ $conversation['unread_count'] }}</span>
                @endif
            </a>
            @endforeach
        </div>
        ...
        <div class="p-4 border-t border-white/5">
            <div class="flex items-center gap-3">
                <img src="{{ Auth::user()->profile_photo ? asset('storage/' . Auth::user()->profile_photo) : 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->name) . '&background=0D8ABC&color=fff' }}" class="w-10 h-10 rounded-full border-2 border-white/10" alt="">
                <div class="flex-1 min-w-0">
                    <h3 class="text-sm font-semibold text-white truncate">{{ Auth::user()->name }}</h3>
                    <p class="text-xs text-slate-400">Online</p>
                </div>
                <a href="{{ route('dashboard') }}" class="p-2 text-slate-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex-1 flex flex-col bg-[#0B0C10]">
        ...
        <!-- Chat Header -->
        <div class="h-16 px-6 bg-[#0F1015] border-b border-white/5 flex items-center justify-between">
            <div class="flex items-center gap-4">
                <div class="flex items-center gap-3">
                    <div class="relative">
                        <img src="{{ $activeChat['user']['avatar'] }}" class="w-10 h-10 rounded-full border-2 border-white/10" alt="">
                        @if($activeChat['user']['online'])
                        <span class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-[#0F1015]"></span>
                        @endif
                    </div>
                    <div>
                        <h3 class="text-sm font-semibold text-white">{{ $activeChat['user']['name'] }}</h3>
                        <p class="text-xs {{ $activeChat['user']['online'] ? 'text-green-500' : 'text-slate-500' }}">
        ...
        <!-- Messages Area -->
        <div id="messages-container" class="flex-1 overflow-y-auto p-6 space-y-4" style="background: linear-gradient(180deg, #0B0C10 0%, #0F1015 100%);">
            ...
            <div class="flex flex-col items-center justify-center h-full text-center">
                ...
                <p class="text-sm text-slate-500 max-w-sm">Send a message to our support team. We're here to help with any questions or issues you may have.</p>
            </div>
            @else
            <!-- Date Separator -->
            <div class="flex justify-center my-4">
                <span class="px-3 py-1 bg-white/5 text-slate-500 text-xs rounded-full border border-white/10">Today</span>
            </div>

            @foreach($activeChat['messages'] as $message)
            <!-- Message -->
            <div class="flex {{ $message['sender'] === 'me' ? 'justify-end' : 'justify-start' }} group animate-fade-in-up">
                <!-- Avatar if 'them' -->
                @if($message['sender'] === 'them')
                <img src="{{ $activeChat['user']['avatar'] }}" class="w-8 h-8 rounded-full mr-3 mt-1 border-2 border-[#0B0C10] self-end mb-1">
                @endif
