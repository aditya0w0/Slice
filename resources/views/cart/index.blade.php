        ...
        <main class="mx-auto max-w-4xl px-6 py-12">
            ...
            @if ($items->isEmpty())
                ...
                <div class="rounded-3xl bg-white p-12 text-center shadow-lg">
                    ...
                    <a
                        href="{{ route('devices') }}"
                        class="mt-6 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-3 font-semibold text-white shadow-lg hover:shadow-xl transition-all"
                    >
            ...
            @else
                ...
                <div class="space-y-4">
                    ...
                    @foreach ($items as $it)
                        ...
                        @endphp
                        <div class="group overflow-hidden rounded-3xl bg-white p-6 shadow-lg transition-all hover:shadow-xl">
                            <div class="flex items-center justify-between">
                                <div class="flex-1">
                                    <h3 class="text-lg font-bold text-gray-900">{{ ucwords(str_replace('-', ' ', $it->variant_slug)) }}</h3>
                                    ...
                                    <div class="mt-3 text-sm text-gray-600">
                                        Rp {{ number_format($it->price_monthly ?? 0, 0, ',', '.') }}/month × {{ $it->months }} months × {{ $it->quantity ?? 1 }} unit
                                    </div>
                                </div>
                ...
                <div class="mt-8 rounded-3xl bg-gradient-to-br from-blue-600 to-purple-600 p-8 shadow-xl">
                    <div class="flex items-center justify-between text-white">
                        <div>
                            ...
                            <p class="mt-2 text-sm text-blue-100">{{ $items->count() }} {{ Str::plural('item', $items->count()) }} in cart</p>
                        </div>
                        <a
