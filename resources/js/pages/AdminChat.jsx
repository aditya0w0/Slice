...
import { Send, Paperclip, Search, MoreVertical, Menu, Smile } from 'lucide-react';
...
export default function AdminChat() {
  ...
  console.log('🔄 AdminChat component rendering, messages count:', messages.length);
  ...
  useEffect(() => {
    ...
    console.log('🎧 Setting up WebSocket listener for user:', activeChat.user.id);
    ...
    const channel = window.Echo.private(`chat.${activeChat.user.id}`)
      .listen('.message.sent', (e) => {
        console.log('📨 New message received via WebSocket on channel chat.' + activeChat.user.id + ':', e);
        ...
        setMessages(prevMessages => {
          ...
          console.log('New messages count will be:', newMessages.length);
      ...
      .error((error) => {
        console.error('❌ Channel subscription error for chat.' + activeChat.user.id + ':', error);
      });
    ...
    channel.subscribed(() => {
      console.log('🎉 Successfully subscribed to channel: chat.' + activeChat.user.id);
    });

    return () => {
      console.log('🔇 Removing WebSocket listener for user:', activeChat.user.id);
  ...
  const fetchChatData = async () => {
    try {
      ...
      if (data.activeChat.messages.length > 0) {
        lastMessageIdRef.current = data.activeChat.messages[data.activeChat.messages.length - 1].id;
      }
      setLoading(false);
    } catch (error) {
  ...
  return (
    <div className="h-screen bg-[#0B0C10] flex overflow-hidden font-sans">
      {/* Sidebar */}
      <div className={`${sidebarOpen ? 'w-80' : 'w-16'} bg-[#0F1015] border-r border-white/5 flex flex-col transition-all duration-300`}>
        {/* Sidebar Header */}
        <div className="p-4 border-b border-white/5 flex items-center justify-between">
          ...
          {sidebarOpen && <h2 className="text-xl font-bold text-white">Support</h2>}
        </div>
          ...
          {contacts.map((contact) => (
            <div
              ...
              className={`flex items-center gap-3 px-4 py-3 hover:bg-white/5 cursor-pointer transition-colors border-l-2 ${
                contact.active ? 'border-blue-500 bg-white/5' : 'border-transparent'
              }`}
            >
              ...
              {sidebarOpen && (
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 truncate">{contact.last_message}</p>
                </div>
              )}
        ...
        <div className="h-16 px-6 bg-[#0F1015] border-b border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">{activeChat?.user?.status}</p>
            </div>
          </div>
          ...
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-sm text-slate-500 max-w-sm">
                Start the conversation by sending a message to the user.
              </p>
            </div>
          ) : (
                ...
                >
                  ...
                  {message.sender === 'me' && (
                    <Avatar className="w-8 h-8 rounded-full ml-3 mt-1 border-2 border-[#0B0C10] self-end mb-1">
                      <AvatarImage src={window.adminUser?.avatar} />
                      <AvatarFallback>{window.adminUser?.name?.[0]}</AvatarFallback>
                    </Avatar>
                  )}
                </div>
              ))}
